...
function Walker(paths, opts, cb) {
  ...
  // resolve . for top-level search paths
  //root = root.map(function(f) {
    //if(f === '.') {
      //return process.cwd(); 
    //}
    //return f;
  ...
  function walk(files, list, cb) {
    ...
    function check(file, cb) {
      ...
      //if(opts.absolute && !path.isAbsolute(file)) {
        //file = path.normalize(path.join()) 
      //}
      ...
      method.call(fs, file, function onStat(err, stats) {
        ...
        if(stats.isDirectory()) {
          ...
          if(!opts.folder(file, info)) {
            ...
            return cb(null, list); 
          }else if(opts.dirs && (!opts.exclude || (opts.exclude && depth > 0))) {
            list.push(info);
          }
          ...
          if(opts.file(file, info)
            && (!opts.exclude || (opts.exclude && !~paths.indexOf(info.file)))) {
            list.push(info);
          }
          return cb(null, list);
        }
